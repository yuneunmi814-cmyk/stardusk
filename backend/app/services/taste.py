...
_DECK_SQL = text(
    ...
    WITH nearby AS (
        ...
        WHERE ST_DWithin(
          ...
          AND spot_name !~ '(시청|군청|구청|도청|행정복지센터|주민센터|행정센터|면사무소|읍사무소|동사무소|보건소|보건지소|우체국|경찰서|파출소|지구대|소방서|세무서|등기소|교육청|법원|노인회|부녀회|청년회|번영회|자치회|협회|지회|연합회|복지관|행정역사관|상권|민화마을|문화유산|유산단지|교육장|수련원|연수원)'  -- 관변·행정 + 도심/시설/문화 제외(사찰·치유의숲·휴양림은 유지)
    )
    SELECT
